...
class AllInOneNode(Node):
        ...
        try:
            ...
            if self.state == STATE_FOLLOWING_PATH:
                ...
                if ids is not None:
                    ...
                    # Estimate pose (rvec = rotation, tvec = translation)
                    ...
                    # Compute docking distance error
                    self.docking_dist_error_m = distance - self.docking_target_dist

                    # --- NEW: Print distance every time marker detected ---
                    self.get_logger().info(f"[Docking] Distance to marker: {distance:.3f} m")

                    self.last_marker_seen_time = self.get_clock().now()
                else:
                    # Optional: handle lost marker
                    self.get_logger().warn("[Docking] Marker lost, stopping.")
                    self.docking_x_error_px = 0.0
                    self.docking_dist_error_m = 0.0
    ...
    def control_loop(self):
        ...
        if self.state == STATE_FOLLOWING_PATH:
            ...
            # Stop when within ±10 cm (0.10 m)
            target_tolerance = 0.10  # <-- desired final distance accuracy

            if abs(self.docking_dist_error_m) < target_tolerance and \
                abs(self.docking_x_error_px) < self.docking_ang_tolerance_px:
                self.get_logger().info(f"✅ Docking complete. Final distance: "
                               f"{self.docking_dist_error_m + self.docking_target_dist:.3f} m")
                twist.linear.x = 0.0
                twist.angular.z = 0.0
                ...
                lin = np.clip(self.kp_linear_dock * self.docking_dist_error_m, -self.max_linear_dock, self.max_linear_dock)
                ang = np.clip(-self.kp_angular_dock * self.docking_x_error_px,  -self.max_angular_dock, self.max_angular_dock)

                twist.linear.x = lin
                twist.angular.z = ang

                self.get_logger().info(f"[Docking] Moving: lin={lin:.3f} ang={ang:.3f} "
                                       f"dist_err={self.docking_dist_error_m:.3f} m")
